 ...
 */
public class NewCacheUtility implements ICacheUtility {

    private static final String TAG = "SqliteUtility_new";

    private static final String KEY_TIME = "com.hawk.funday.cache.new.KEY_TIME";
    ...
    public IResult findCacheData(Setting setting, Params params) {
        ...
        if (TextUtils.isEmpty(topId) && TextUtils.isEmpty(bottomId)) {//不是刷新也不是加载更多，才去读缓存
            ...
            try {
                list = FundayDB.getCacheDB().select(PostBean.class, "com_m_common_key = ?", new String[]{"new"}, null, null, "id desc", null);
            } catch (Exception e) {
                e.printStackTrace();
            }
