...
/**
 * @author Liyang Sun
 * @Description: Profile posted 页面缓存
 * @date 2016/9/12 15:36
 * @copyright HAWK
 */
public class ProfilePostedCacheUtility implements ICacheUtility {

    private static final String TAG = "ProfilePostedCacheUtility";

    private static final String KEY_TIME = "com.hawk.funday.cache.profile.posted.KEY_TIME";
    ...
    public IResult findCacheData(Setting setting, Params params) {
        ...
        if (offset.equals(String.valueOf(0))) { // offset 为0才加载缓存，不是0不加载
            ...
            if (list != null && list.size() != 0) {
                ...
                Logger.d(TAG, "findcache, list size = %d", list.size());
                return result;
            }
        }
